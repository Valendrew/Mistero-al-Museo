    ...
    if (accedi) {
      return (
        <div>
          ...
          <Button variant="link" onClick={() => setAccedi(false)}>Oppure clicca qui per registrarti!</Button>
        </div>
      )
    } else {
      return (
        <div>
          ...
          <Button variant="link" onClick={() => setAccedi(true)}>Oppure clicca qui per Accedere!</Button>
        </div>)
    }
  }
  const handleSubmit = (event) => {
    ...
    if (accedi) {
      fetch('/auth/login', {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: username, password: sha256(pass).toString() })
      }).then(res => {
        if (res.status === 200) {
    ...
    <Container >
      ...
      <Form onSubmit={(e) => handleSubmit(e)}>
        <Form.Group controlId="formEmail">
          <Form.Label for="usr">Inserisci nome utente:</Form.Label>
        ...
        <Form.Group controlId="formEmail">
          <Form.Label for="pwd">Inserisci la password:</Form.Label>
          <Form.Control required minLength="4" type="password" id="pwd" value={pass}onChange={(event) => setPass(event.target.value)}></Form.Control><br/>
        </Form.Group>
        <Bottone />
